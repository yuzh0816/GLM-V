...
class GLM4VModel:
    ...
    def _stream_fragment(self, buf):
        if "```html" in buf.lower():
            think_content = ""
            if "<think>" in buf:
                if "</think>" in buf:
                    seg = re.search(r"<think>(.*?)</think>", buf, re.DOTALL)
                    if seg:
                        think_content = f"💭 **Thinking Process:**\n\n{seg.group(1).strip()}\n\n---\n\n"
                else:
            ...
            if "<answer>" in buf:
                if "</answer>" in buf:
                    ...
                    if seg:
                        answer_content = seg.group(1).strip()
                else:
                    answer_content = buf.split("<answer>", 1)[1]
        ...
        if "<think>" in buf:
            if "</think>" in buf:
                seg = re.search(r"<think>(.*?)</think>", buf, re.DOTALL)
                if seg:
                    think_html = f"""<details open><summary style='cursor:pointer;font-weight:bold;color:#bbbbbb;'>💭 Thinking Process</summary><div style='color:#cccccc;line-height:1.4;padding:10px;border-left:3px solid #666;margin:5px 0;background-color:rgba(128,128,128,0.1);'>{seg.group(1).strip()}</div></details>"""
            else:
                part = buf.split("<think>", 1)[1]
                think_html = f"""<details open><summary style='cursor:pointer;font-weight:bold;color:#bbbbbb;'>💭 Thinking Process</summary><div style='color:#cccccc;line-height:1.4;padding:10px;border-left:3px solid #666;margin:5px 0;background-color:rgba(128,128,128,0.1);'>{part}</div></details>"""
