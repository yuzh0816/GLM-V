...
class RobustInference:
    ...
    def generate_with_force_completion(
        ...
        inputs = self._prepare_inputs(messages, video_path).to(self.model.device)
        input_length = inputs["input_ids"].shape[1]

        first_generated_ids = self.model.generate(
            **inputs, max_new_tokens=first_max_tokens, do_sample=do_sample, temperature=temperature
        )

        first_output_ids = first_generated_ids[0][input_length:]

        needs_completion = self._check_needs_completion_by_tokens(first_output_ids, first_max_tokens)

        if not needs_completion:
            final_output = self.processor.decode(first_output_ids, skip_special_tokens=False)
            return {
        ...
        force_input_ids = self._prepare_force_input(inputs["input_ids"], first_output_ids)
        ...
        second_generated_ids = self.model.generate(
            **force_inputs, max_new_tokens=force_max_tokens, do_sample=do_sample, temperature=temperature
        )
        ...
        complete_output_ids = torch.cat([first_output_ids, added_tokens, second_output_ids], dim=0)
        complete_output_text = self.processor.decode(complete_output_ids, skip_special_tokens=False)
    ...
    def _prepare_inputs(self, messages, video_path=None):
        text = self.processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)

        if video_path:
            video_tensor, video_metadata = load_video(video_path)
            inputs = self.processor(
                text=text, videos=[video_tensor], video_metadata=[video_metadata], return_tensors="pt"
            )
        else:
    ...
    def _prepare_force_input(self, original_input_ids, first_output_ids):
        ...
        if not has_think_end:
            tokens_to_add.extend([self.special_tokens["think_end"], self.special_tokens["answer_start"]])
        ...
        if tokens_to_add:
            ...
            force_input_ids = torch.cat([original_input_ids, first_output_ids.unsqueeze(0), additional_tokens], dim=1)
        else:
            force_input_ids = torch.cat([original_input_ids, first_output_ids.unsqueeze(0)], dim=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="THUDM/GLM-4.1V-9B-Thinking", help="Model path")
    parser.add_argument("--video_path", type=str, required=True, help="Video file path")
    parser.add_argument("--first_max_tokens", type=int, default=8192, help="First generation max tokens")
    parser.add_argument("--force_max_tokens", type=int, default=8192, help="Force completion max tokens")
