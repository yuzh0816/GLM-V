...
def test_physics_verifier_judge_physical_direction_difference_invalid(physics_verifier):
    ...
    assert physics_verifier.judge(
        "左侧的合力为5N，右侧为3N，所以合力向右",
        "左侧合力为5N，右侧为3N，因此合力向左",
        question=question
    ) == 0.0
...
def test_physics_verifier_judge_physical_ordered_list_should_match(physics_verifier):
    ...
    assert physics_verifier.judge("磁通量变化→产生感应电动势→形成感应电流", "产生感应电动势→磁通量变化→形成感应电流", question=question) == 0.0
    
    # 光电效应过程顺序错误
    question = "光电效应的基本过程依次是？"
    assert physics_verifier.judge("光子入射→电子吸收能量→电子逸出", "电子逸出→光子入射→电子吸收能量", question=question) == 0.0
    
    # 简谐振动一个周期的顺序错误
    question = "弹簧振子从平衡位置开始一个完整周期的运动顺序？"
    assert physics_verifier.judge("平衡位置→最大位移→平衡位置→反向最大位移→平衡位置", "最大位移→平衡位置→反向最大位移→平衡位置→平衡位置", question=question) == 0.0
    
    # 机械波传播过程顺序错误
    question = "机械波的产生和传播过程依次是？"
    assert physics_verifier.judge("振源振动→介质质点振动→波动传播", "波动传播→振源振动→介质质点振动", question=question) == 0.0
    
    # α衰变过程顺序错误
    question = "原子核α衰变的过程步骤是？"
    assert physics_verifier.judge("不稳定核→发射α粒子→形成新核", "形成新核→不稳定核→发射α粒子", question=question) == 0.0

    # 易错补充
    question = "实验步骤？"
    assert physics_verifier.judge("准备→操作→记录→分析", "准备 操作 记录 分析", question=question) == 1.0
    question = "电磁波的传播顺序？"
    assert physics_verifier.judge("电场变化→磁场变化→电磁波传播", "电场变化 磁场变化 电磁波传播", question=question) == 1.0
    assert physics_verifier.judge("电场变化→磁场变化→电磁波传播", "磁场变化 电场变化 电磁波传播", question=question) == 0.0

def test_physics_verifier_judge_physical_ordered_list_match(physics_verifier):
    # 顺序正确
    question = "声音传播的基本过程？"
    assert physics_verifier.judge("振动→产生声波→传播→人耳接收", "振动 产生声波 传播 人耳接收", question=question) == 1.0

    # 牛顿运动定律应用步骤正确
    question = "用牛顿第二定律解题的基本步骤？"
    assert physics_verifier.judge("受力分析→建立坐标系→列方程→求解", "受力分析 建立坐标系 列方程 求解", question=question) == 1.0
    
    # 电路分析步骤正确
    question = "分析复杂电路的基本步骤？"
    assert physics_verifier.judge("简化电路→确定等效电阻→计算总电流→分析各支路", "简化电路 确定等效电阻 计算总电流 分析各支路", question=question) == 1.0
    ...
    assert physics_verifier.judge("单色光源→单缝→双缝→屏幕观察", "单色光源 单缝 双缝 屏幕观察", question=question) == 1.0
