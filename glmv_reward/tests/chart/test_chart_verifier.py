...
def test_extract_answer(chart_verifier):
    ...
    response = "<think>The answer is 1+x.</think><|begin_of_box|>1+x<|end_of_box|> and <|begin_of_box|>2+y<|end_of_box|>"
...
def test_chart_verifier_judge_Ambiguous_answers(chart_verifier):
    ...
    assert chart_verifier.judge("\\text{label from the graph (identified by visual inspection or analysis of bar heights)}", "1.0", question=question) == 0.0
    question = "What is the average of the highest and lowest value of AVG ?"
    assert chart_verifier.judge("\\text{labels from the bar graph where 1971 values have the specified difference (specific labels based on graph data)}", "0.28", question=question) == 0.0
    question = "What is the sum of the average of 1963.0  and the average of 1961.0 ?"
    assert chart_verifier.judge("\\text{l\\text{value from graph (based on visual data)}", "-5207080204.63", question=question) == 0.0

def test_chart_verifier_judge_year_question(chart_verifier):
    question = "Which x-axis label has the minimum difference between Dominica and Belize" #question中不出现year等关键词
    assert chart_verifier.judge("2010", "2010", question=question) == 1.0
    assert chart_verifier.judge("2010", "2011", question=question) == 0.0
    question_with_year = "Which year has the minimum difference between Dominica and Belize" #question中出现year等关键词
